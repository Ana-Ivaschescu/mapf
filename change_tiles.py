...
directory_path = r'C:\mapformer\data/HRSCD/change/'
out_dir = r'C:\mapformer\data/preprocessed/500'

for d in ['D14', 'D35']:
    files = os.listdir(osp.join(directory_path, d))
    for file in tqdm(files):
        name, suffix = file.split('.')
        file_path = osp.join(directory_path, d, file) 
        raster_array, src = read_tif_to_array(file_path)
        os.makedirs(osp.join(out_dir, 'change', d, name), exist_ok=True)
        output_folder = osp.join(out_dir, 'change', d, name)
